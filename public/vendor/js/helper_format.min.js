...
const CirebonwebFormat = {
    /**
     * Memformat tanggal dari YYYY-MM-DD menjadi DD-MM-YYYY.
     *
     * Contoh:
    ...
    Tanggal: function (value, mode = 'auto') {
        ...
        // Nilai MySQL dengan jam memakai pemisah spasi atau "T" setelah tanggal.
        const hasTime = /^\d{4}-\d{2}-\d{2}[T\s]\d{2}:\d{2}/.test(valueString);
        if (mode === 'auto' && !hasTime) return tanggal;
    ...
    /**
     * Formatter kolom Rupiah untuk DataTables.
     * Menampilkan prefix "Rp" hanya untuk tampilan dan pencarian,
     * tetapi mempertahankan nilai asli untuk sorting dan export.
     *
     * Contoh:
     * { targets: [5, 6], render: CirebonwebFormat.RupiahTabel },
     *
     * @param {string|number} value Nilai angka.
     * @param {string} type Tipe render DataTables.
     * @returns {string|number} Nilai terformat atau nilai asli.
     */
    RupiahTabel: function (value, type) {
        if (type === 'display' || type === 'filter') {
            return 'Rp ' + CirebonwebFormat.Rupiah(value);
        }
        return value;
    },
